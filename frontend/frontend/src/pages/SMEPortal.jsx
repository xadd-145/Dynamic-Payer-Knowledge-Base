...
export default function SMEPortal() {
  ...
  const handleApprove = async (f) => {
    ...
    if (!fd.rule_code || !fd.topic_code || !fd.effective_start_date) {
      setMsg("Fill in Rule Code, Topic Code, and Effective Start Date.");
      return;
    }
    try {
      ...
      setMsg(`✓ Rule ${fd.rule_code} published.`);
      loadQueue();
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-purple-800 text-white px-6 py-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-purple-200">TIER 3 Escalated Fragment Resolution</p>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        {!loading && fragments.length === 0 && (
          <div className="bg-white rounded-xl border p-8 text-center">
            <p className="text-gray-500 font-semibold">No fragments in TIER 3 queue</p>
            <p className="text-xs text-gray-400 mt-1">Nothing to resolve.</p>
          </div>
        )}
        ...
        <div className="space-y-6">
          {fragments.map(f => {
            const fd = form[f.policy_fragment_id] || {};
            return (
              <div key={f.policy_fragment_id} className="bg-white rounded-xl border-2 border-purple-200 shadow-sm p-5">
                <div className="flex justify-between items-center mb-3">
                  <span className="bg-purple-100 text-purple-800 text-xs font-black px-2 py-1 rounded">
                    TIER 3 · Score: {f.confidence_score} · SME REQUIRED
                  </span>
                ...
                <div className="text-xs text-gray-500 mb-4 space-y-1">
                  ...
                  <p><span className="font-semibold">Extracted date:</span> {f.extracted_effective_start_date ?? "None — SME must supply"}</p>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-3">
                  {[
                    ["Rule Code", "rule_code", "e.g. OBS-001"],
                    ["Rule Name", "rule_name", "e.g. Observation Revenue Code"],
                    ["Topic Code", "topic_code", "e.g. OBS"],
                    ["Effective Start", "effective_start_date", "YYYY-MM-DD"],
                    ["Effective End", "effective_end_date", "YYYY-MM-DD or blank"],
                    ["Interpretation Notes", "notes", "Required for TIER 3"],
                    ...
                    <div key={field}>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-purple-500"
                      />
                    </div>
                  ))}
                </div>
